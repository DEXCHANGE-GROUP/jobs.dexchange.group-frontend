...
export default function CandidateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (c?.resumeUrl) {
        const key = s3KeyFromUrl(c.resumeUrl);
          ...
          <div className="bg-white border border-border rounded-xl p-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
                ...
                {c.resumeUrl && cvDownloadUrl && (
                  <div className="flex items-center gap-2.5 sm:col-span-2">
                    <svg className="w-4 h-4 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                    </svg>
                    <a href={cvDownloadUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Voir le CV</a>
                  </div>
                )}
              </div>
            </div>
